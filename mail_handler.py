...
def wait_element(driver, by, value, timeout=10):
    ...
    # Manual wait (polling)
    poll_interval = 0.2  # Faster polling
...
def _find_target_mail_row(driver, target_subject, rows=None):
    ...
    # Prioritize unread mails first, then fallback to all if not found
    unread_rows = []
    read_rows = []
    for idx, row in enumerate(rows):
        ...
        if row.find_elements(By.TAG_NAME, "th"):
            continue
        try:
            if _row_is_unread(row):
                unread_rows.append((idx, row))
            else:
                read_rows.append((idx, row))
        except Exception:
            continue

    def scan_rows(row_list):
        for idx, row in row_list:
            try:
                ...
                if is_instagram and is_target_subj:
                    ...
                    print(f"     [Row {idx}] Unread/Read but logic mismatch: Instagram={is_instagram}, Subj='{target_subject}' -> {is_target_subj}")
            except Exception as e:
    ...
    # Try unread first
    target = scan_rows(unread_rows)
    if target:
        return target
    # Fallback: try read mails if nothing found
    return scan_rows(read_rows)
...
def _click_mail_row(driver, row) -> None:
    ...
    try:
        ...
        # Không sleep lâu, chỉ chờ rất ngắn
        time.sleep(0.05)
        # Ưu tiên click JS, nếu không được thì click thường
        target = None
        try:
        ...
        # Sau khi click, chờ rất ngắn rồi lấy luôn text
        time.sleep(0.1)
    except Exception as e:
    ...
    try:
        ...
        for i in range(5):
            try:
                ...
                driver.switch_to.default_content()
                _wait_dom_ready(driver, timeout=8)  # Không refresh đầu vòng lặp
                rows = _wait_for_mail_rows(driver, timeout=6)  # Reduce wait
                if not rows:
                ...
                if not target_row:
                    ...
                    # Chỉ refresh khi không tìm thấy mail phù hợp
                    if not _safe_refresh(driver):
                ...
                # Sau khi click, lấy luôn text từ email_content hoặc body, tìm code bằng regex
                print("   [Mail] Reading mail content (fast)...")
                code = None
                try:
                    # Ưu tiên lấy text từ email_content
                    div = driver.find_element(By.ID, "email_content")
                    text = div.text
                except:
                    text = ""
                if not text:
                    try:
                        ...
                        text = ""
                if text:
                    code = extract_instagram_code(text)
                if code:
                    print(f"   [Mail] -> FOUND CODE (fast): {code}")
                    return code
                else:
                    print("   [Mail] Code not found in fast text scan. Trying fallback (deep scan)...")
                    # Nếu vẫn không thấy, fallback về logic cũ (deep scan)
                    ...
                    final_code = None
                    end_time = time.time() + 6  # Reduce wait
